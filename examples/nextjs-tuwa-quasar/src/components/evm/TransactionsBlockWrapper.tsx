...
export const TransactionsBlockWrapper = () => {
    ...
    await executeTxAction({
      ...
      onSuccess: (tx) => {
        if (tx.type === TxType.increment) {
          ...
          const hash = 'hash' in tx && typeof tx.hash === 'string' ? tx.hash : undefined;
          notifyTransactionSuccess({ txKey: tx.txKey, hash });
  ...
  return (
    <div className="w-full max-w-md h-auto min-h-[680px] bg-[var(--tuwa-bg-primary)] rounded-[var(--tuwa-rounded-corners)] shadow-2xl border border-[var(--tuwa-border-primary)] overflow-hidden flex flex-col relative">
      ...
      <div className="flex-1 p-8 space-y-8">
        ...
        <div className="space-y-6 flex-1">
          <div className="bg-[var(--tuwa-bg-secondary)] rounded-[var(--tuwa-rounded-corners)] p-4 border border-[var(--tuwa-border-secondary)] min-h-[100px] flex flex-col justify-center">
            <div className="flex flex-wrap items-center justify-between gap-2 w-full mb-3">
              ...
              <div className="text-right ml-auto flex-shrink-0">
                <div className="text-xs text-[var(--tuwa-text-secondary)] leading-tight">Contract</div>
                <button
      ...
      <footer className="bg-[var(--tuwa-bg-secondary)] px-4 sm:px-8 py-3.5 border-t border-[var(--tuwa-border-primary)] flex-shrink-0 min-h-[56px] flex items-center justify-center">
        <div className="flex flex-wrap items-center justify-center gap-1.5 text-xs text-[var(--tuwa-text-tertiary)] text-center">
          <span className="leading-tight">Powered by</span>
          <a href="https://github.com/TuwaIO" target="_blank" rel="noopener noreferrer">
            <span className="font-semibold text-[var(--tuwa-text-accent)] leading-tight">TUWA</span>
          </a>
          <span className="leading-tight">•</span>
          <span className="leading-tight">Web3 Transaction Tracking</span>
        </div>
      </footer>
    </div>
  );
